    ...
    def get(self, request):
        ...
        if page == 0 or page is None:
            ...
            if result:
                ...
                return JsonResponse({'message': '没有商品订单噢~', 'code':-1})
...
class UserInfoView(View):
    ...
    def get(self, request):
        ...
        sql = "SELECT CustomerId Id,CustomerName Name,CustomerProfilePhoto Img,CustomerLV Lv,CustomerAddress Address,CustomerBalance Balance,CustomerTELE TELE,CustomerEmail Email FROM customer WHERE CustomerId = %s"
        cursor.execute(sql, [userId])
        info = cursor.fetchall()
        print('hahaha')
        if info:
            info = info[0]
            LV = ['红海会员', '银海会员', '金海会员', '银海会员']
    ...
    def post(self, request):
        ...
        if operation == 1:
            ...
            sql = 'UPDATE customer SET CustomerName = %s, CustomerEmail = %s, CustomerAddress = %s, CustomerTELE = %s WHERE CustomerID = %s'
            cursor.execute(sql, [Name, Email, Address, TELE, userId])
            conn.commit()
            ...
            sql = 'UPDATE customer SET CustomerPassword = %s WHERE CustomerID = %s'
            cursor.execute(sql, [newPwd, userId])
            conn.commit()
            return JsonResponse({'code': 0})
...
class goodsDetail(View):
    def get(self, request):
        ...
        conn = mysql.connector.connect(host='localhost', user='root', password='root',
                                       ...
                                       auth_plugin="mysql_native_password")
        cursor = conn.cursor(dictionary=True)
        sql = 'SELECT * FROM Commodity WHERE CommodityID = %s'
        cursor.execute(sql,[commodity_id])
        result = cursor.fetchall()

        return JsonResponse({'data':result[0],'code':0})
...
class getGoodsList(View):
    def get(self,request):
        ...
        cursor = conn.cursor(dictionary=True)
        if typeId == -1:
            sql = 'SELECT CommodityID id, CommodityProfilePhoto img, CommodityName name, CommodityPrice price,CommodityType typeid FROM commodity'
            cursor.execute(sql)
            result = cursor.fetchall()
            if result:
                data = {
...
class getTypes(View):
    def get(self,request):
        ...
        cursor = conn.cursor(dictionary=True)
        sql = 'SELECT DISTINCT CommodityType id FROM commodity GROUP BY CommodityType'
        cursor.execute(sql)
        result = cursor.fetchall()
        ...
        if result:
            ...
            for i in range(len(result)):
                result[i]['name'] = label[int(result[i]['id'])-1]
...
class getRecommendList(View):
    def get(self,request):
        ...
        cursor = conn.cursor(dictionary=True)
        sql = 'SELECT  c.CommodityID id,c.CommodityProfilePhoto img, c.CommodityName name, c.CommodityPrice unitprice, s.discount, s.startdate, s.enddate FROM commodity c, recommendation r,salespromotion s WHERE r.CustomerID = %s AND r.CommodityID = c.CommodityID AND s.SalesPromotionID = c.SalesPromotionID'
        cursor.execute(sql, [userId])
        result = cursor.fetchall()
        if result:
            for i in range(len(result)):
                if datetime.date.today() < result[i]['startdate'].today()  or result[i][
                    'enddate'].today() < datetime.date.today():
                    result[i]['discount'] = 1.0
            data = {'data': result,
                    'code': 0}
            return JsonResponse(data)
        else:
            sql = 'SELECT  c.CommodityID id,c.CommodityProfilePhoto img, c.CommodityName name, c.CommodityPrice unitprice, s.discount, s.startdate, s.enddate FROM commodity c,salespromotion s WHERE s.SalesPromotionID = c.SalesPromotionID ORDER BY c.CommoditySalesVolume DESC'
            cursor.execute(sql)
            result = cursor.fetchall()
            if result:
                for i in range(len(result)):
                    if datetime.date.today() < result[i]['startdate'].today() or result[i]['enddate'].today() < datetime.date.today():
                        result[i]['discount'] = 1.0
                data = {'data': result,
                        'code': 0}
                return JsonResponse(data)
...
class searchGoods(View):
    def get(self,request):
        ...
        cursor = conn.cursor(dictionary=True)
        key = '%'+key+'%'
        sql = "SELECT CommodityID id,CommodityName name,CommodityProfilePhoto img,CommodityPrice price FROM commodity WHERE CommodityName LIKE %s "
        ...
        if result:
            ...
            return JsonResponse({'message':'无符合要求的商品','code': -1})
...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        img = 'https://gimg2.baidu.com/image_search/src=http%3A%2F%2Fc-ssl.duitang.com%2Fuploads%2Fitem%2F202004%2F10%2F20200410212830_wtvje.thumb.1000_0.jpg&refer=http%3A%2F%2Fc-ssl.duitang.com&app=2002&size=f9999,10000&q=a80&n=0&g=0n&fmt=jpeg?sec=1643387746&t=c48b5e717b72a29efe665bc64c1b4e5f'
        ...
        conn = mysql.connector.connect(host='localhost', user='root', password='root', database='RUCtronic',
                                       auth_plugin="mysql_native_password")
        cursor = conn.cursor()
        cursor.execute("INSERT INTO customer(CustomerId,CustomerName,CustomerPassword,CustomerLv,CustomerBalance,CustomerAddress,Province,CustomerEmail, CustomerTELE, CustomerProfilePhoto)"
                       "values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", [username,nickname, password, 0, 0,address, region, email, phone, img])
        conn.commit()
...
class Orderform(View):
    ...
    def get(self, request):
        ...
        if is_type == 0:  # 如果是买家
            ...
            if state == -1:
                ...
                sql = 'SELECT Comment comment, CustomerID customer FROM Orderform WHERE CommodityID = %s and IsRating = 1'
                cursor.execute(sql, [commodityId])
                '''逐行读取行，若没有数据返回为空'''
                result = cursor.fetchall()

                if result:
                    sql = 'SELECT AVG(Rating) rating FROM Orderform WHERE CommodityID = %s and IsRating = 1'
                    cursor.execute(sql, [commodityId])
                    '''逐行读取行，若没有数据返回为空'''
                    rating = cursor.fetchall()

                    return JsonResponse({'data': {'comment':result,'rating':rating}, 'code': 0})
                else:
                    return JsonResponse({'message': '没有商品订单噢~','code': -1})

            elif state >= 0 and state <= 3:
                sql = "SELECT o.OrderID id,o.OrderDate createtime,o.OrderState state,c.CommodityProfilePhoto img,c.Commodity_description spec,c.CommodityID com_id,c.CommodityName com_name,c.CommodityPrice unitPrice,o.Quantity num,o.Quantity*o.UnitPrice amount,o.IsRating FROM Orderform o, Commodity c WHERE o.CommodityID = c.CommodityID AND o.CustomerID = %s AND o.OrderState = %s"
                cursor.execute(sql, [cus_id, state])
                '''逐行读取行，若没有数据返回为空'''
                result = cursor.fetchall()
            ...
            if result:
                ...
                #     result[i]['state'] = int(getDictKey(state_map,state)[0])
                return JsonResponse({'data': result, 'code': 0})
            else:
                return JsonResponse({'message': '没有商品订单噢~', 'code':-1})
            conn.close()
    ...
    def post(self, request):
        ...
        if is_type == 0:  # 买家
            ...
            # cus_update： -1：下单 0：添加购物车 1:添加评价 3：签收
            # 下单
            if cus_update == -1:
                commodityId = data_json.get('goodsDetailId')
                sellerId = data_json.get('sellerId')
                commodityNum = data_json.get('num')
                # 订单自增id由商品ID+买家ID+当前时间组成
                present_time = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
                orderId = commodityId + user.username + present_time
                ...
                # 判断商品库存
                CommodityInventory = comm_data['CommodityInventory']
                if CommodityInventory < commodityNum:
                    return JsonResponse({'errmsg': 'Exceeds maximum inventory'})

                # 获取促销活动信息
                promotionId = comm_data['SalesPromotionID']
                sql = 'SELECT discount FROM SalesPromotion WHERE SalesPromotionID = %s'
                cursor.execute(sql, [promotionId])
                promotionRate = cursor.fetchone()
                if promotionRate:
                    unitPrice = promotionRate['discount'] * comm_data['CommodityPrice']
                else:
                    unitPrice = comm_data['CommodityPrice']

                # 判断个人余额是否足够
                sql = 'SELECT CustomerBalance FROM Customer WHERE CustomerID = %s'
                cursor.execute(sql, [user.username])
                balance = cursor.fetchone()
                if unitPrice * commodityNum > balance['CustomerBalance']:
                    return JsonResponse({'errmsg': 'There is not enough balance! Please recharge', 'code': -1})
                else:
                    charged_balance = balance['CustomerBalance'] - unitPrice * commodityNum

                sql = 'INSERT INTO Orderform(OrderID,SellerID,CommodityID,CustomerID,Quantity,UnitPrice,OrderDate,OrderState,IsRating) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)'
                cursor.execute(sql,
                               [orderId, comm_data['SellerID'], comm_data['CommodityID'], user.username, commodityNum,
                                unitPrice, order_date, 0, 0])
                conn.commit()

                sql = 'UPDATE seller SET SellerIncome = SellerIncome + %s WHERE SellerID = %s'
                cursor.execute(sql, [unitPrice * commodityNum, sellerId])
                conn.commit()

                sql = 'UPDATE Customer SET CustomerBalance = %s WHERE CustomerID = %s'
                cursor.execute(sql, [charged_balance, user.username])
                conn.commit()
            ...
            # 添加购物车
                ...
                goodsDetailId = data_json.get('goodsDetailId')
                num = data_json.get('num')

                sql = 'SELECT * FROM shoppingcart WHERE CustomerID = %s and CommodityID = %s'
                cursor.execute(sql, [user.username, goodsDetailId])
                cart_data = cursor.fetchone()
                # 如果商品已存在于购物车中，增加商品数量
                if cart_data != None:
                    num += cart_data['CommodityQuantity']
                    sql = 'UPDATE shoppingcart SET CommodityQuantity = %s WHERE CustomerID = %s and CommodityID = %s'
                    cursor.execute(sql, [num, user.username, goodsDetailId])
                    conn.commit()
                else:
                    sql = 'INSERT INTO shoppingcart VALUES(%s,%s,%s)'
                    cursor.execute(sql, [user.username, goodsDetailId, num])
                    conn.commit()
            ...
            # 增加评价
                ...
                rating = data_json.get('score')

                sql = 'UPDATE Orderform SET IsRating = %s, Rating = %s, Comment = %s WHERE OrderID = %s'
                cursor.execute(sql, [1, rating, comment, orderId])
                conn.commit()
            ...
            # 签收
                ...
                # 获取订单ID
                orderid = data_json.get('orderId')  # 获取订单id
                # 根据订单ID对Orderform表格中的Orderstate作出修改
                # 首先获取当前的订单状态
                sql = 'SELECT Orderstate FROM Orderform WHERE OrderID = %s'
                cursor.execute(sql, [orderid])
                orderstate = cursor.fetchone()
                if orderstate == 3:  # 如果当前订单状态是作废，则不能修改
                    return JsonResponse({'update': 'No permission'})
                else:  # 若当前订单状态不是作废，则可以修改
                    sql = 'UPDATE Orderform SET Orderstate = %s WHERE OrderID = %s'
                    cursor.execute(sql, [2, orderid])
                    conn.commit()
                    return JsonResponse({'update': 'Receipt', 'code': 0})

            else:  # 如果买家执行的不是收货，则不被允许
                return JsonResponse({'update': 'No permission'})
            conn.close()
    ...
    def get(self, request):
        ...
        if result:
            ...
            for i in range(len(result)):
                ...
                sql = 'SELECT s.discount discount FROM salespromotion s,Commodity c,Shoppingcart sh WHERE s.SalesPromotionID = c.SalesPromotionID AND c.CommodityID = sh.CommodityID AND sh.CustomerID = %s AND %s>=s.startdate AND %s<=s.enddate AND c.CommodityID = %s'
                cursor.execute(sql, [cus_id, now_time, now_time,result[i]['id']])
            ...
            for i in range(len(cartList)):
                ...
                if com_num > inventory:
                    ...
                    if balance < amount:
                        ...
                        sql = 'UPDATE commodity SET CommoditySalesVolume = CommoditySalesVolume + %s WHERE CommodityID = %s'
                        cursor.execute(sql, [com_num, com_id])
                        conn.commit()

                        sql = 'UPDATE customer SET CustomerBalance = CustomerBalance - %s WHERE CustomerID = %s'
                        ...
                        sql = 'INSERT INTO orderform(OrderID,SellerID,CommodityID,CustomerID,Quantity,UnitPrice,OrderDate,OrderState,IsRating) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s)'
                        cursor.execute(sql, [orderid, sel_id, com_id, cus_id, com_num, amount, now_time, 0,0])
                        conn.commit()
...
class SalesPromotion(View):
    def get(self, request):
        ...
        SalesPromotionId = request.GET.get('id')
        sql = 'SELECT SalesPromotionID, SalesPromotionName, startdate, enddate, discount FROM SalesPromotion sp WHERE sp.SalesPromotionID = %s'
